...
"""
Samples a large number of sequences from a pre-trained DiT model using DDP.
Subsequently saves a .npz file that can be used to compute evaluation metrics.
...
def create_npz_from_sample_folder(sample_dir, num=50_000):
    """
    Builds a single .npz file from a folder of samples.
    """
    samples = []
    for i in tqdm(range(num), desc="Building .npz file from samples"):
        sample_np = np.load(f"{sample_dir}/{i:06d}.npy")
...
def main(args):
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-sequence-length-{args.sequence_length}-" \
    ...
    if rank == 0:
        print(f"Total number of sequences that will be sampled: {total_samples}")
    ...
    for _ in pbar:
        ...
        # Save samples to disk as individual .npy files
